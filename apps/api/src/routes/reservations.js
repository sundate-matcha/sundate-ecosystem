...
router.post('/', validateReservation, async (req, res) => {
  try {
    ...
    // Check availability
    const availability = await Reservation.checkAvailability(date, time, guests);
...
router.put('/:id', validateReservation, async (req, res) => {
  try {
    ...
    if (req.body.date !== reservation.date || req.body.time !== reservation.time || req.body.guests !== reservation.guests) {
